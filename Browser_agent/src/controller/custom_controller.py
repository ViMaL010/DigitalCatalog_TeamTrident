...
from src.agent.deep_research.formmanage import ask_user
import json
import pyperclip
...
class CustomController(Controller):
    ...
    def _register_custom_actions(self):
        ...
        async def ask_for_assistant(query: str, browser: BrowserContext):
            """
            Always ask the user for input using voice, regardless of query format.
            """
            try:
                print(f"[DEBUG] Received query: {query}")
                print(f"[DEBUG] Calling ask_user with prompt: {query}")
                result = ask_user(query)
                print(f"[DEBUG] ask_user returned: {result}")
                # Always wrap the result in a dict for consistency
                return ActionResult(
                    extracted_content=json.dumps({"user_input": result}),
                    include_in_memory=True,
                )
            except Exception as e:
                logger.error(f"[ask_for_assistant Error]: {e}", exc_info=True)
                print(f"[DEBUG] Exception occurred in ask_for_assistant: {e}")
                return ActionResult(
                    extracted_content=f"Error while asking assistant: {str(e)}",
                    include_in_memory=True,
                )
    ...
    async def act(
        ...
        try:
            ...
            logger.error(f"Error during act execution: {e}", exc_info=True)
            raise e
